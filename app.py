...
GEMINI_API_KEY = "AIzaSyDX5kilAqDyNTmwhgW8_vMUa90TrHb2WTA"
...
def get_summary(pipe, text):
    try:
        if len(text.split()) < 30:
            return None
        max_l = min(100, max(25, len(text.split()) // 2))
        r = pipe(text[:1024], max_length=max_l, min_length=15, do_sample=False)
...
def gemini_reply(history, user_msg, sentiment_label):
    try:
        ...
        system = f"You are a professional and helpful AI assistant. {tone} Keep responses concise and clear."
        chat_history = [
...
with st.sidebar:
    ...
    st.markdown('<div class="sb-sub">Gemini API &nbsp;·&nbsp; HuggingFace Transformers</div>', unsafe_allow_html=True)
    ...
    <div class="stat-grid">
        ...
        <div class="stat-box">
            <div class="stat-num" style="color:#4ADE80">{st.session_state.pos}</div>
            <div class="stat-lbl">Positive</div>
        </div>
        <div class="stat-box">
            <div class="stat-num" style="color:#F87171">{st.session_state.neg}</div>
...
<div class="page-hdr">
    ...
    <div class="page-hdr-sub">Powered by Gemini 1.5 Flash &nbsp;·&nbsp; 5 HuggingFace Transformer models running in real-time</div>
</div>
""", unsafe_allow_html=True)
...
if send and user_input.strip():
    ...
    with st.spinner("Generating response..."):
        reply = gemini_reply(st.session_state.messages[:-1], user_input, s_label)
...
if not st.session_state.analyses:
    ...
    with col1:
        ...
        col  = "#22C55E"   if lbl == "POSITIVE" else "#EF4444"   if lbl == "NEGATIVE" else "#F59E0B"
        st.markdown(f"""
        <div class="acard">
            ...
            <div class="prog-bg"><div class="prog-fill" style="width:{score*100:.0f}%;background:{col};"></div></div>
        </div>""", unsafe_allow_html=True)
    ...
    with col2:
        if a["intent"]:
            ...
            st.markdown(f"""
            <div class="acard">
                <div class="acard-label">Intent Classification &nbsp;<span class="sb-mono">bart-mnli</span></div>
                {rows}
            </div>""", unsafe_allow_html=True)
        ...
        st.markdown(f"""
        <div class="acard">
            <div class="acard-label">Named Entity Recognition &nbsp;<span class="sb-mono">bert-conll03</span></div>
    ...
    with col3:
        if a["summary"]:
            st.markdown(f"""
            <div class="acard">
                <div class="acard-label">Auto Summary &nbsp;<span class="sb-mono">distilbart-cnn</span></div>
